...
from ..dependencies import get_request_hub, get_tool_session_service, verify_casefile_access
...
async def create_session(
    ...
    # Create new session for casefile via RequestHub
...
async def close_session(
    ...
    return await hub.dispatch(close_request)
...
async def list_available_tools(
    ...
    if category:
        ...
        if enabled_only:
            tool_dict = {
                name: tool for name, tool in tool_dict.items() if tool.business_rules.enabled
            }

    return {
        "tools": [tool.to_discovery_format() for tool in tool_dict.values()],
        "count": len(tool_dict),
    }
...
async def get_tool_parameter_schema(tool_name: str) -> dict[str, Any]:
    ...
    return {"tool_name": tool_name, "schema": tool.get_openapi_schema()}
