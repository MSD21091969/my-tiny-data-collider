...
async def sheets_batch_get(
    ...
    from src.pydantic_ai_integration.google_workspace import SheetsClient
    from src.pydantic_ai_integration.google_workspace import SheetsBatchGetRequest
    
    # Initialize client
    client = SheetsClient(user_id=ctx.user_id)
    
    # Build request
    request = SheetsBatchGetRequest(
        spreadsheet_id=spreadsheet_id,        ranges=ranges    )
    response = await client.batch_get(request=request)
