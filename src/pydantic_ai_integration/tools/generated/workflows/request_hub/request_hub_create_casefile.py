...
class RequesthubcreatecasefileParams(BaseModel):
    ...
    description: Optional[str] = Field('', description="Optional descriptive text for the casefile.")
    tags: Optional[List[Any]] = Field([], description="Labels applied to the new casefile.")
    hook_channels: Optional[List[Any]] = Field(['metrics', 'audit'], description="Additional hook identifiers to activate.")

    class Config:
        json_schema_extra = {
            "examples": [
                {
                    "title": 'example',                    "description": '',                    "tags": [],                    "hook_channels": ['metrics', 'audit']                }
            ]
        }
...
async def request_hub_create_casefile(
    ctx: MDSContext,
    title: str,    description: str = '',    tags: List[Any] = [],    hook_channels: List[Any] = ['metrics', 'audit']) -> Dict[str, Any]:
    """Create a casefile through RequestHub with hook execution and policy defaults.
