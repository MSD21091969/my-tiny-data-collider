...
class RequesthubcreatecasefilewithsessionParams(BaseModel):
    ...
    auto_start_session: Optional[bool] = Field(
        True, description="Whether to automatically start a tool session after casefile creation."
    )
    ...
    hook_channels: Optional[List[Any]] = Field(
        ["metrics", "audit"], description="Hook identifiers to activate during workflow execution."
    )
...
async def request_hub_create_casefile_with_session(
    ...
    response = await service.execute_casefile_with_session(
        title=title,
        description=description,
        tags=tags,
        auto_start_session=auto_start_session,
