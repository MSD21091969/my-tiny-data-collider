...
from ..base.custom_types import EmailAddress, FileSizeBytes, IsoTimestamp, ShortString, UrlString


class DriveOwner(BaseModel):
    """Metadata describing a file owner."""

    email: EmailAddress = Field(..., description="Owner email")
    display_name: Optional[ShortString] = Field(None, description="Owner display name")


class DriveFile(BaseModel):
    """Representation of a Google Drive file."""

    id: ShortString = Field(..., description="Drive file ID")
    name: ShortString = Field(..., description="File name")
    mime_type: ShortString = Field(..., description="MIME type")
    size_bytes: Optional[FileSizeBytes] = Field(None, description="File size in bytes")
    web_view_link: Optional[UrlString] = Field(None, description="Link for viewing the file")
    icon_link: Optional[UrlString] = Field(None, description="Icon link for the file type")
    parents: List[ShortString] = Field(default_factory=list, description="Parent folder IDs")
    owners: List[DriveOwner] = Field(default_factory=list, description="File owners")
    created_time: IsoTimestamp = Field(default_factory=lambda: datetime.now().isoformat(), description="Creation timestamp")
    modified_time: IsoTimestamp = Field(default_factory=lambda: datetime.now().isoformat(), description="Last modification timestamp")
...
class DriveFolder(BaseModel):
    ...
    id: ShortString = Field(..., description="Drive folder ID")
    name: ShortString = Field(..., description="Folder name")
    parents: List[ShortString] = Field(default_factory=list, description="Parent folder IDs")
    created_time: IsoTimestamp = Field(default_factory=lambda: datetime.now().isoformat(), description="Creation timestamp")
    modified_time: IsoTimestamp = Field(default_factory=lambda: datetime.now().isoformat(), description="Last modification timestamp")
...
class CasefileDriveData(BaseModel):
    ...
    last_sync_token: Optional[ShortString] = Field(None, description="Token for incremental Drive sync")
