...
class CreateCasefileWithSessionPayload(BaseModel):
    ...
    description: str | None = Field(None, max_length=2000, description="Casefile description")
    tags: list[str] = Field(default_factory=list, description="Tags for categorization")
    auto_start_session: bool = Field(True, description="Whether a tool session should be created")
    session_title: str | None = Field(None, description="Optional explicit session title")
...
class CasefileWithSessionResultPayload(BaseModel):
    ...
    session_id: str | None = Field(None, description="Created session identifier when requested")
...
class CreateSessionWithCasefilePayload(BaseModel):
    ...
    casefile_id: str = Field(..., description="Existing casefile ID to link session to")
    title: str | None = Field(None, description="Optional session title")
    session_type: str = Field("tool", description="Type of session to create (tool or chat)")
...
class SessionWithCasefileResultPayload(BaseModel):
    ...
    casefile_id: str = Field(..., description="Casefile ID the session was linked to")
    session_type: str = Field(..., description="Type of session created")
