...
class DTOAlignmentValidator:
    ...
    def validate_rar_pattern(self):
        ...
        for method_name, method_def in methods.items():
            ...
            # Skip external integrations - they don't follow internal R-A-R pattern
            if method_def.integration_tier in ["external", "hybrid"]:
                continue

            # Skip private utility methods - they don't follow R-A-R pattern
            if method_def.name.startswith("_"):
                continue
