...
class ToolDefinitionValidator:
    ...
    def validate_tool_file(self, tool_path: Path) -> bool:
        ...
        try:
            ...
            self.errors.append(ValidationError(tool_path.stem, "ERROR", f"YAML parse error: {e}"))
            return False

        if not tool_def:
            self.errors.append(ValidationError(tool_path.stem, "ERROR", "Empty tool definition"))
            return False
        ...
        if request_model_name:
            self._validate_request_model_alignment(
                tool_name, tool_def, request_model_name, data_contracts.get("module")
            )
            ...
            if not impl_type:
                ...
                self.errors.append(
                    ValidationError(
                        tool_name,
                        "ERROR",
                        f"Invalid implementation.type: {impl_type}",
                        field="implementation.type",
                    )
                )
                ...
                if payload_field and hasattr(payload_field.annotation, "model_fields"):
                    ...
                    for field_name, field_info in payload_fields.items():
                        if field_info.is_required():
                            matching_param = next(
                                (p for p in method_params if p["name"] == field_name), None
                            )
    ...
    def _validate_version_metadata(self, tool_name: str, tool_def: Dict[str, Any]):
        ...
        if tool_version and not self._is_valid_semver(tool_version):
            self.warnings.append(
                ValidationError(
                    tool_name, "WARNING", f"Invalid semver format: {tool_version}", field="version"
                )
            )
        ...
        if not method_version:
            self.warnings.append(
                ValidationError(
                    tool_name,
                    "WARNING",
                    "Missing compatibility.method_version",
                    field="compatibility.method_version",
                )
            )
        ...
        if not schema_version:
            self.warnings.append(
                ValidationError(
                    tool_name,
                    "WARNING",
                    "Missing compatibility.schema_version",
