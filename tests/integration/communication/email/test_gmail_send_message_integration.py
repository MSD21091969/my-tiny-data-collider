"""Auto-generated integration tests for: gmail_send_message
...
@pytest.mark.asyncio
class TestIntegrationGmailsendmessage:
    """Integration tests for gmail_send_message through service layer."""
    ...
    async def test_scenario_send_with_cc(self, service, test_session):
        ...
        assert response.status == RequestStatus.COMPLETED
        assert "message_id" in response.payload.result
        assert response.payload.result["message_id"] == mock-sent-123


@pytest.mark.asyncio
class TestIntegrationGmailsendmessagePolicies:
    """Test policy enforcement for gmail_send_message at service layer."""
