...
from src.authservice.token import create_token, get_current_user, create_service_token
...
class TestMVPUserJourney:
    ...
    @pytest.mark.asyncio
    async def test_complete_user_journey_create_casefile_and_execute_tool(self):
        """
        ...
        assert decoded["session_id"] == session_id        # Step 2: Create casefile (would typically call CasefileService)
        # For MVP, we validate the request structure
        ...
        assert casefile_request.user_id == user_id
        assert casefile_request.payload["title"] == "MVP Test Casefile"

        # Step 3: Create tool session (would typically call ToolSessionService)
        ...
        assert session_request.payload["casefile_id"] == casefile_id
        ...
        assert tool_request.user_id == user_id
        assert tool_request.metadata["session_id"] == session_id
        assert tool_request.metadata["casefile_id"] == casefile_id

        # Verify routing metadata present for audit trail
        assert "session_request_id" in tool_request.metadata
    ...
    def test_service_token_flow_for_automation(self):
        ...
        decoded = get_current_user(service_token)
        assert decoded["sub"] == f"service:{service_name}"
        assert decoded["casefile_id"] == casefile_id
        
        # Service tokens can create operations without human user
        # This supports automation scripts and background jobs    @pytest.mark.asyncio
    async def test_session_context_preservation_across_operations(self):
