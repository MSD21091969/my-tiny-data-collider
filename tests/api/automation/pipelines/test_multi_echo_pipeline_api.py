"""Auto-generated API tests for: multi_echo_pipeline
...
@pytest.mark.api
class TestAPIMultiechopipeline:
    """API tests for multi_echo_pipeline through HTTP endpoints."""
    ...
    def test_parameter_validation_via_api(self, client, auth_headers, test_session):
        ...
        request_data = {
            ...
            "payload": {
                "tool_name": "multi_echo_pipeline",
                "parameters": {
                    "initial_message": 12345,  # Wrong type - should be string
                }
            }
        }
...
@pytest.mark.api
class TestAPIMultiechopipelineToolRegistry:
    """Test tool registry endpoints for multi_echo_pipeline."""
    
    def test_tool_listed_in_registry(self, client):
        """Test that multi_echo_pipeline appears in /tools/ endpoint."""
        ...
        assert "multi_echo_pipeline" in tool_names
    
    def test_get_tool_definition(self, client):
        """Test GET /tools/{tool_name} endpoint."""
        response = client.get("/tools/multi_echo_pipeline")
        
        assert response.status_code == 200
        tool_def = response.json()
        assert tool_def["name"] == "multi_echo_pipeline"
